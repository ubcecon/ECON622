...
# Setup:
# Make sure to setup an account on wandb.ai, connect your login to github is easiest
# Then run `wandb login` in the terminal and login to your account
...
# Initialize W&B
wandb.init(project="linear_regression_pytorch")
...
# Define the model (Linear Regression with M features)
class LinearRegressionModel(nn.Module):
    def __init__(self, M):
        super(LinearRegressionModel, self).__init__()
        self.layer_1 = nn.Linear(M, 1, bias=False)

    def forward(self, x):
        return self.layer_1(x)


model = LinearRegressionModel(M)
...
print(
    f"Test Loss: {test_loss:.4g}, ||theta - theta_hat|| = {torch.norm(theta - model.state_dict()['layer_1.weight'])}"
)
wandb.log(
    {
        "test_loss": test_loss,
        "theta_diff": torch.norm(theta - model.state_dict()["layer_1.weight"]),
    }
)
wandb.finish()
