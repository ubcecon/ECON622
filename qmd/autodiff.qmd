...
db = zero(β0)
@btime Enzyme.autodiff(Enzyme.ReverseWithPrimal,logit_likelihood, Active, Duplicated(β0,db), Const(y), Const(x))
db
```

```{julia}
using LinearAlgebra
v = Tuple(Float64.(I(length(β0))[:,j]) for j in 1:length(β0))
@btime dbf = Enzyme.autodiff(Enzyme.ForwardWithPrimal, logit_likelihood, BatchDuplicated(β0,v), Const(y), Const(x))
# or
...
- See [Enzyme.jl FAQ](https://enzyme.mit.edu/index.fcgi/julia/stable/faq/) if getting strange errors
...
- [Reactant.jl](https://github.com/EnzymeAD/Reactant.jl) compiles Julia to MLIR
...
- [Mooncake.jl](https://chalk-lab.github.io/Mooncake.jl/dev/#Mooncake.jl) performant reverse and forward mode AD supporting arbitrary Julia code
  - a few [known limitation](https://chalk-lab.github.io/Mooncake.jl/dev/known_limitations/)
...
- Represent calculation symbolicly, compute symbolic derivative, convert to numeric function
- Best used to differentiate relatively small functions
- [Symbolics.jl](https://docs.sciml.ai/Symbolics/stable/)
  - More used, better tested
- [FastDifferentiation.jl](https://github.com/brianguenter/FastDifferentiation.jl)
  - Faster, but [fails in complex cases](https://github.com/brianguenter/FastDifferentiation.jl/issues/65)

## FastDifferentiation.jl

```{julia}
import FastDifferentiation as FD
βs=FD.make_variables(:β, length(β0))
FD.@variables ys
xs = FD.make_variables(:x, length(β0))
...
@btime ∇logit_likelihood_FD(β0,y,x)
```
...
[Tracker](https://github.com/FluxML/Tracker.jl) is a tape based reverse mode package. It was the default autodiff package in Flux before being replaced by Zygote. No longer under active development, but solid and still used.
...
[Diffractor](https://github.com/JuliaDiff/Diffractor.jl) is an automatic differentiation package in development. It was once hoped to be the future of AD in Julia, but has been delayed. It plans to have both forward and reverse mode, but only forward mode is available so far.
